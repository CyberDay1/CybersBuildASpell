...
- The cross-mod reference documents the casting API at last, and corrects two attribute notes: a duplicate modifier throws rather than being ignored, and total multipliers compound. The configuration reference gains the eighteen settings it never listed. The config's per-enchantment caps are described everywhere as capping the bonus scaling, never what level the altar sells. The 26.1 copy's lookup snippet now compiles.
- Spell Power never scaled mana cost, whatever three pages said: the held wand's discount, read from either hand, is the one cast-time reduction. Sanctuary's page states its level III blessing cap, and the mana pages note a server may hand the pool to another magic mod, taking the two mana enchantments off sale.
