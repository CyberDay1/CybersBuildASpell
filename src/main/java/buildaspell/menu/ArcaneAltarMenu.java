...
public class ArcaneAltarMenu extends AbstractContainerMenu {
    ...
    // First player-inventory slot index in this.slots (after the 5 container slots).
    ...
    public ArcaneAltarMenu(int containerId, Inventory playerInv, Container container) {
        ...
        // Capped at the item's own stack size so bumping the container limit (for ingredients)
        // doesn't let the enchant target overfill past its natural max.
        ...
        // Ingredient slots — a centred 2x2 grid (4 slots, one per material tier) living in the
        // side tab (local coords sit to the right of the main GUI). Hidden until the tab opens.
        // Each accepts up to five vanilla stacks (bypassing the per-item 64 clamp) so a big bill fits.
     ...
     * ever leave the altar: vanilla's NBT item codec caps a count at 99, so one sitting in a player
    ...
    public void clicked(int slotId, int button, ClickType clickType, Player player) {
        if (clickType == ClickType.SWAP && slotId >= 0 && slotId < CONTAINER_SLOTS) {
