...
public final class BuildASpellAPI {
     ...
     * <b>ordered</b> list of component ids. Order is meaningful for effect-bound modifiers — such a
     * modifier binds to the effect that precedes it — so the components must be one interleaved
     * list, not a split of effects/modifiers.
     *
     * <p>Delivery-level (roster) modifiers ({@code double, split, accelerate, pierce, bounce,
     * return, chain, delay, echo, linger, duration}) attach to the delivery as a whole and are
     * sorted out of the component stream into the spell's delivery-modifier list automatically, so
     * their position in {@code componentIds} does not matter. External callers keep passing one flat
     * id list exactly as before.
     *
