...
public class WandItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, TooltipDisplay display,
        ...
        tooltipAdder.accept(Component.translatable("item.buildaspell.wand.tooltip.cast").withStyle(ChatFormatting.GRAY));
        tooltipAdder.accept(Component.translatable("item.buildaspell.wand.tooltip.mana", reductionPct).withStyle(ChatFormatting.AQUA));
        if (power > 0) {
            String shown = (power == Math.floor(power)) ? String.valueOf((int) power) : String.valueOf(power);
            tooltipAdder.accept(Component.translatable("item.buildaspell.wand.tooltip.power", shown).withStyle(ChatFormatting.LIGHT_PURPLE));
        }
    }
