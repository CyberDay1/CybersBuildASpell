...
public class SpellExporter {
    ...
    public static String toNeoOriginsPower(Spell spell) {
        ...
        // Delivery-level (roster) modifiers are carried in the same flat id list that
        // BuildASpellAPI.createSpell consumes; it sorts them back out into the spell's
        // delivery-modifier bucket on import, so their position here does not matter. We emit
        // them first, then the effect-first component stream (whose ordering is a locked design).
        for (SpellModifier deliveryMod : spell.getDeliveryModifiers()) {
            components.add(deliveryMod.getSerializedName());
        }
    ...
    public static String encode(Spell spell) {
        ...
        // Components in order, preserving interleaved effect/modifier structure. Delivery-level
        // modifiers are emitted first as m: entries; on decode they funnel through addComponent,
        // which routes roster modifiers back into the spell's delivery-modifier bucket.
        List<String> componentEntries = new ArrayList<>();
        for (SpellModifier deliveryMod : spell.getDeliveryModifiers()) {
            componentEntries.add("m:" + deliveryMod.getSerializedName());
        }
        for (SpellComponent component : spell.getComponents()) {
