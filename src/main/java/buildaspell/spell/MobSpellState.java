...
public final class MobSpellState {
    ...
    public static boolean isExpiredSummon(Mob mob) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(SUMMON_EXPIRY_KEY) && mob.level().getGameTime() > data.getLongOr(SUMMON_EXPIRY_KEY, 0L);
    }
    ...
    public static boolean isSummonerOf(Mob mob, LivingEntity target) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(SUMMONER_KEY) && data.getStringOr(SUMMONER_KEY, "").equals(target.getUUID().toString());
    }
    ...
    public static boolean isPacified(Mob mob) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(PACIFIED_UNTIL_KEY) && mob.level().getGameTime() < data.getLongOr(PACIFIED_UNTIL_KEY, Long.MAX_VALUE);
    }
}
