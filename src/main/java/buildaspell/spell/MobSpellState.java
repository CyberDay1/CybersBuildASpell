...
public final class MobSpellState {
    ...
    public static boolean isExpiredSummon(Mob mob) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(SUMMON_EXPIRY_KEY) && mob.level().getGameTime() > data.getLong(SUMMON_EXPIRY_KEY);
    }
    ...
    public static boolean isSummonerOf(Mob mob, LivingEntity target) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(SUMMONER_KEY) && data.getString(SUMMONER_KEY).equals(target.getUUID().toString());
    }
    ...
    public static boolean isPacified(Mob mob) {
        CompoundTag data = mob.getPersistentData();
        return data.contains(PACIFIED_UNTIL_KEY) && mob.level().getGameTime() < data.getLong(PACIFIED_UNTIL_KEY);
    }
}
