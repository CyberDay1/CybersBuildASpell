...
public enum SpellCombo {
    ...
    public boolean matches(Spell spell) {
        ...
        // Delivery-level modifiers (chain, duration, ...) now live in their own bucket, out of
        // getComponents(); count both so combos that require them still clear minComponents.
        int totalComponents = spell.getComponents().size() + spell.getDeliveryModifiers().size();
