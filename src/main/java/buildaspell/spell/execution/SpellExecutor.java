...
public class SpellExecutor {
    ...
    public static class EffectGroup {
        ...
        public final net.minecraft.resources.ResourceLocation dataEffectId;
        public final Map<SpellModifier, Integer> modifiers;
        /**
         * How many identical copies of this effect were folded together. Always at least 1; only
         * {@link #mergeRepeats} ever raises it, and only for consecutive groups carrying the same
         * modifiers.
         */
        public int stacks = 1;
        ...
        public EffectGroup(net.minecraft.resources.ResourceLocation dataEffectId) {
            this.effect = null;
    ...
    private static List<EffectGroup> parseOrderedComponents(Spell spell) {
        ...
        for (SpellComponent component : components) {
            if (component instanceof SpellComponent.Effect eff) {
                ...
                // Roster (delivery-level) modifiers are read globally from the spell, not bound to an
                // effect. Migration normally moves them out of components at load; skip any stray one
                // that slipped through rather than mis-binding it to an effect.
                if (ModifierApplicability.isDeliveryLevel(mod.modifier())) {
                    continue;
                }
        ...
        return mergeRepeats(groups);
    }

    /**
     * Folds runs of the same effect into one group carrying a stack count.
     *
     * <p>Only <em>consecutive</em> groups merge, and only when their modifier maps are equal. Both
     * limits are deliberate. Merging across a gap would reorder the spell, which matters for effects
     * whose position in the list is the point (a Teleport between two other effects is not the same
     * spell as both Teleports up front). Merging groups with different modifiers would silently
     * relabel them, since modifiers bind to the effect they follow: {@code Damage, Damage+Sunder} is
     * two different attacks, not one doubled attack.
     *
     * <p>Effects that cannot express a stack count still behave exactly as before, because
     * {@link #executeEffectWithModifiers} runs those groups {@code stacks} times.
     */
    private static List<EffectGroup> mergeRepeats(List<EffectGroup> groups) {
        List<EffectGroup> merged = new ArrayList<>(groups.size());
        for (EffectGroup group : groups) {
            EffectGroup previous = merged.isEmpty() ? null : merged.get(merged.size() - 1);
            if (previous != null
                    && previous.effect == group.effect
                    && Objects.equals(previous.dataEffectId, group.dataEffectId)
                    && previous.modifiers.equals(group.modifiers)) {
                previous.stacks += group.stacks;
            } else {
                merged.add(group);
            }
        }
    ...
    /** Copy of the spell with every instance of the given modifier removed from BOTH buckets. */
    private static Spell stripModifier(Spell spell, SpellModifier modifier) {
        ...
        List<SpellModifier> filteredDelivery = new ArrayList<>();
        for (SpellModifier mod : spell.getDeliveryModifiers()) {
            if (mod != modifier) filteredDelivery.add(mod);
        }
        return new Spell(spell.getDelivery(), filtered, filteredDelivery, spell.getVisual());
    }
    ...
    private static void spawnRuneMarker(Player caster, Spell spell) {
        ...
        Vec3 offset = Vec3.atLowerCornerOf(aim.direction.getNormal()).scale(0.01);
    ...
    private static void executeEffectWithModifiers(Player caster, Level level, Vec3 origin, EffectGroup group, Spell originalSpell, float spellPower) {
        ...
        // Carry every folded copy into the temp spell, not just one. Nothing reads the effect back out
        // to size its behavior, but ECHO replays this spell wholesale, so a merged group has to look
        // like the run of effects it came from or the replay would land weaker than the original cast.
        for (int i = 0; i < group.stacks; i++) {
        ...
        if (group.effect == SpellEffect.DAMAGE) {
            // Damage is the one effect that folds: it runs once and is told how many copies it stands
            // for, so the stack lands as a single blow. Everything else repeats exactly as it always
            // has, because an effect that cannot express a count would otherwise fire once instead of
            // once per copy.
            executeEffect(caster, level, origin, group.effect, tempSpell, spellPower, group.stacks);
        } else if (group.effect != null) {
            for (int i = 0; i < group.stacks; i++) {
                executeEffect(caster, level, origin, group.effect, tempSpell, spellPower, 1);
            }
        } else {
            for (int i = 0; i < group.stacks; i++) {
    ...
    private static void executeEffect(Player caster, Level level, Vec3 origin, SpellEffect effect, Spell spell, float spellPower) {
        executeEffect(caster, level, origin, effect, spell, spellPower, 1);
    }

    private static void executeEffect(Player caster, Level level, Vec3 origin, SpellEffect effect, Spell spell, float spellPower, int stacks) {
        ...
        if (def != null) {
            def.run(new buildaspell.spell.data.SpellContext(caster, level, origin, spell, spellPower, stacks));
            return;
        }
        runBuiltinEffectBehavior(caster, level, origin, effect, spell, spellPower, stacks);
    }
     ...
     */
    private static void executeDataEffect(Player caster, Level level, Vec3 origin, net.minecraft.resources.ResourceLocation id, Spell spell, float spellPower) {
    ...
    public static void runBuiltinEffectBehavior(Player caster, Level level, Vec3 origin, SpellEffect effect, Spell spell, float spellPower) {
        runBuiltinEffectBehavior(caster, level, origin, effect, spell, spellPower, 1);
    }

    /**
     * @param stacks how many folded copies of {@code effect} this call stands for. Only Damage reads
     *               it; every other effect is invoked once per copy by the caller instead.
     */
    public static void runBuiltinEffectBehavior(Player caster, Level level, Vec3 origin, SpellEffect effect, Spell spell, float spellPower, int stacks) {
        switch (effect) {
            case DAMAGE -> EffectExecutors.executeDamage(caster, level, origin, spell, spellPower, stacks);
