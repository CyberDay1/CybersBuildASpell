 ...
 * </ul>
 */
public record ComponentDisplay(Optional<ResourceLocation> icon, Optional<Integer> color,
    ...
    public static final Codec<ComponentDisplay> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            ResourceLocation.CODEC.optionalFieldOf("icon").forGetter(ComponentDisplay::icon),
    ...
    public static final StreamCodec<ByteBuf, ComponentDisplay> STREAM_CODEC = StreamCodec.of(
            (buf, d) -> {
                ByteBufCodecs.optional(ResourceLocation.STREAM_CODEC).encode(buf, d.icon);
