 ...
 */
public class ComboRegistry extends SimplePreparableReloadListener<Map<Identifier, JsonElement>> {
    ...
    private static final List<Map.Entry<Identifier, ComboDefinition>> ORDERED = new ArrayList<>();
    ...
    protected void apply(Map<Identifier, JsonElement> objects, ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<Identifier, ComboDefinition> parsed = new HashMap<>();
    ...
    public static ComboDefinition detect(Spell spell) {
        if (!ORDERED.isEmpty()) {
            for (Map.Entry<Identifier, ComboDefinition> entry : ORDERED) {
