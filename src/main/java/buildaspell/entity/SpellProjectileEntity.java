...
public class SpellProjectileEntity extends Projectile {
    ...
    protected void readAdditionalSaveData(CompoundTag input) {
        ...
        this.isTracking = input.contains("IsTracking") ? input.getBoolean("IsTracking") : false;
        this.lifetime = input.contains("Lifetime") ? input.getInt("Lifetime") : 0;
        this.blocksPierced = input.contains("BlocksPierced") ? input.getInt("BlocksPierced") : 0;
        this.bouncesRemaining = input.contains("BouncesRemaining") ? input.getInt("BouncesRemaining") : 0;
        this.returning = input.contains("Returning") && input.getBoolean("Returning");
        this.hitEntityIds.clear();
        if (input.contains("HitEntities")) {
            for (Tag t : input.getList("HitEntities", Tag.TAG_STRING)) {
                try {
                    this.hitEntityIds.add(java.util.UUID.fromString(t.getAsString()));
    ...
    protected void addAdditionalSaveData(CompoundTag output) {
        ...
        if (spell != null) {
            output.put("Spell", Spell.CODEC.encodeStart(NbtOps.INSTANCE, spell).getOrThrow());
        }
