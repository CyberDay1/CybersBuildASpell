...
public class BlizzardEntity extends Entity {
    private static final int ICE_BLUE = 0x99CCFF; // (153, 204, 255)
    ...
    public BlizzardEntity(Level level, @org.jetbrains.annotations.Nullable java.util.UUID casterUUID,
                          Vec3 position, float radius, int duration,
                          float spellPower, boolean nullify) {
    ...
    public void tick() {
        ...
        if (!nullify && lifetime % buildaspell.config.ModConfig.comboInt("blizzard", "damageIntervalTicks", 20) == 0) {
            ...
            if (coldDamage > 0) {
                ...
                // The 26.2 BlizzardEntity doesn't track its caster (the freeze pass above filters on
                // canFreeze() only), so the damage pass matches that and just requires a live target.
    ...
    private void spawnSnow(ServerLevel level) {
        ...
        // (ICE_BLUE is a packed 0xRRGGBB int on 26.2's DustParticleOptions ctor)
    ...
    protected void readAdditionalSaveData(ValueInput input) {
        this.radius = input.getFloatOr("Radius", 5.0f);
        ...
        input.getString("CasterUUID").ifPresent(s -> {
            try {
                this.casterUUID = java.util.UUID.fromString(s);
