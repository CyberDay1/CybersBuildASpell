...
public class ModConfig {
    ...
    public static class Wands {
        ...
        public Wands(ModConfigSpec.Builder builder) {
            ...
            builder.comment("Three wand tiers. While a wand is held it discounts the mana cost of every spell",
                    ...
                    .push("wands");
            define(builder, WandTier.WORN, 0.10, 6.0);
            define(builder, WandTier.CARVED, 0.25, 15.0);
            define(builder, WandTier.RUNIC, 0.40, 30.0);
            builder.pop();
        }
    ...
    // ---- Balance migrations ------------------------------------------------------------------
    /**
     * The balance pass {@code effects.toml} is expected to be at. Bump this and add a case to
     * {@link #migrateEffects()} whenever a shipped default changes, or the new number will only
     * ever be seen by worlds created after the update.
     */
    private static final int EFFECTS_CONFIG_VERSION = 1;

    /** The pre-1.0.1 Damage scaling, retired because a Runic wand made a single Damage a one-shot. */
    private static final double LEGACY_DAMAGE_SPELL_POWER_SCALE = 0.25;

    /**
     * The balance pass {@code wands.toml} is expected to be at. Separate from the effects version
     * because it is a separate file: they are written, read and migrated independently.
     */
    private static final int WANDS_CONFIG_VERSION = 1;

    /**
     * The pre-1.0.1 wand Spell Power bonuses. Retired because they were additive on a base attribute
     * of 10, which put bare-handed casting and a Runic wand 8.5x apart — a spread no single damage
     * scale can sit sensibly in the middle of.
     */
    private static final Map<WandTier, Double> LEGACY_WAND_SPELL_POWER = Map.of(
            WandTier.WORN, 10.0,
    ...
    /**
     * Brings a config file written by an older version up to {@link #EFFECTS_CONFIG_VERSION}.
     *
     * <p>Each step only rewrites a value that is still sitting on the default it is replacing: a
     * server owner who deliberately tuned the number keeps their choice, while everyone who never
     * opened the file gets the new balance. Bumping the version is what makes this run once.
     */
    private static void migrateEffects() {
        if (EFFECTS == null) return;
        int from = EFFECTS.configVersion.get();
        if (from >= EFFECTS_CONFIG_VERSION) return;

        if (from < 1) {
            ModConfigSpec.DoubleValue scale = EFFECTS.tuneD.get("damage.spellPowerScale");
            if (scale != null && scale.get() == LEGACY_DAMAGE_SPELL_POWER_SCALE) {
                scale.set(scale.getDefault());
                BuildASpell.LOGGER.info("Retired the pre-1.0.1 Damage scaling: damage.spellPowerScale {} -> {}",
                        LEGACY_DAMAGE_SPELL_POWER_SCALE, scale.getDefault());
            }
        }

        EFFECTS.configVersion.set(EFFECTS_CONFIG_VERSION);
        EFFECTS_SPEC.save();
    }

    /**
     * Brings {@code wands.toml} up to {@link #WANDS_CONFIG_VERSION}, on the same terms as
     * {@link #migrateEffects()}: a tier is only rewritten while it still holds the exact number the
     * old release shipped, so a server owner who retuned a wand keeps their value.
     */
    private static void migrateWands() {
        if (WANDS == null) return;
        int from = WANDS.configVersion.get();
        if (from >= WANDS_CONFIG_VERSION) return;

        if (from < 1) {
            LEGACY_WAND_SPELL_POWER.forEach((tier, legacy) -> {
                ModConfigSpec.DoubleValue power = WANDS.spellPower.get(tier);
                if (power != null && power.get().equals(legacy)) {
                    power.set(power.getDefault());
                    BuildASpell.LOGGER.info("Retired the pre-1.0.1 {} wand Spell Power: {} -> {}",
                            tier.getSerializedName(), legacy, power.getDefault());
                }
            });
        }
