...
public class ArcaneAltarScreen extends AbstractContainerScreen<ArcaneAltarMenu> {
    ...
    protected void init() {
        ...
        manaPoolBtn = addRenderableWidget(ThemedButton.of(
                ...
                btn -> selectEnchantment(Identifier.fromNamespaceAndPath("buildaspell", "mana_pool"))));
        manaPoolBtn.setTooltip(Tooltip.create(Component.literal("Increases maximum mana capacity")));

        manaRegenBtn = addRenderableWidget(ThemedButton.of(
                Component.literal("Mana Regen"), bx, topPos + 42, bw, bh,
                btn -> selectEnchantment(Identifier.fromNamespaceAndPath("buildaspell", "mana_regeneration"))));
        manaRegenBtn.setTooltip(Tooltip.create(Component.literal("Increases mana regeneration rate")));

        spellPowerBtn = addRenderableWidget(ThemedButton.of(
                Component.literal("Spell Power"), bx, topPos + 62, bw, bh,
                btn -> selectEnchantment(Identifier.fromNamespaceAndPath("buildaspell", "spell_power"))));
        spellPowerBtn.setTooltip(Tooltip.create(Component.literal("Increases spell damage and effect strength")));
    ...
    protected void containerTick() {
        super.containerTick();
        if (enchantBtn != null) {
            // Clickable as soon as a type is chosen and an item sits in the altar. The
            // server validates XP + materials and messages the player if something's
            // short — so the button never silently greys out on them.
            enchantBtn.active = selectedEnchantment != null && !menu.getSlot(0).getItem().isEmpty();
        }
    }
    ...
    /**
     * Draws a grayed-out preview of each slot's fixed material whenever the ingredient tab is open —
     * regardless of whether a type is selected — so the player always sees which material each slot
     * wants. Only empty slots show a ghost; a filled slot shows its real stack instead.
     */
    private void renderIngredientGhosts(GuiGraphicsExtractor graphics) {
        ...
        for (int i = 0; i < ArcaneAltarBlockEntity.INGREDIENT_COUNT; i++) {
            ...
            // Advance a stratum so the translucent wash lands above the ghost item (26.1's GUI
            // render state layers by stratum; there is no z-translate on the 2D pose stack).
    ...
    private void renderHelpPanel(GuiGraphicsExtractor graphics) {
        ...
        addWrapped(lines, "Place a wand or item in the altar, pick a type, and set a target level.", contentW);
        addWrapped(lines, "", contentW);
