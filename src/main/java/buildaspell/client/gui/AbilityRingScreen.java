...
public class AbilityRingScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (hasSpell) {
                ...
                graphics.text(font, name, slotX - textWidth / 2, slotY - 6, 0xFFFFFFFF, true);

                // Slot-number badge, bottom-right corner.
                String num = String.valueOf(i + 1);
                graphics.text(font, num, x1 - BORDER - font.width(num) - 2, y1 - BORDER - 9, 0xFFC8C8D2, true);
            } else {
                String label = String.valueOf(i + 1);
                int textWidth = font.width(label);
                graphics.text(font, label, slotX - textWidth / 2, slotY - 4, 0xFFBCBCC6, true);
            }
        }
        ...
        if (hoveredSlot >= 0) {
            ...
            if (slot != null && slot.hasSpell()) {
                ...
                // Keep the body fully opaque so the slot labels underneath can't bleed through.
                ...
                for (int j = 0; j < lines.size(); j++) {
                    graphics.text(font, lines.get(j), tx, ty + j * 10, 0xFFFFFFFF, false);
                }
            } else {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean forwarded) {
        if (event.button() == 0 && hoveredSlot >= 0) {
            if (minecraft != null && minecraft.player != null) {
                minecraft.player.playSound(SoundEvents.UI_BUTTON_CLICK.value(), 1.0f, 1.0f);
            }
            ClientPacketDistributor.sendToServer(new SetActiveSlotPacket(hoveredSlot));
            this.onClose();
            return true;
        }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (slot >= 0 && slot < 10) {
            ...
            ClientPacketDistributor.sendToServer(new SetActiveSlotPacket(slot));
            this.onClose();
            return true;
        }
