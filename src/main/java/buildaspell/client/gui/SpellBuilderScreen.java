...
public class SpellBuilderScreen extends Screen {
    ...
    private enum SlotKind { DELIVERY, EFFECT, MODIFIER, ADD_MODIFIER, ADD_EFFECT }
    ...
    protected void init() {
        ...
        nameField.setValue(lastSpellName);
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        ...
        if (dragging == null) {
            ...
            if (hovered == null) {
                ...
                if (box != null) hovered = componentInBox(box);
            }
            if (hovered != null) renderComponentTooltip(g, hovered, mouseX, mouseY);
        }
    }
    ...
    private List<PalRow> buildPaletteRows() {
        ...
        addPaletteSection(rows, y, "palette_modifiers", MODIFIER_ACCENT, c -> c instanceof SpellModifier);
        return rows;
    }
    ...
    private void renderPalette(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        for (PalRow r : rows) {
            ...
            if (r.isHeader()) {
                ...
                if (hovered) g.fill(innerX, ry, rightX, ry + r.h() - 1, SLOT_HOVER);
                drawSlot(g, innerX, ry, 18, r.accent(), false);
                drawIcon(g, r.component(), innerX + 1, ry + 1);
                drawNameFit(g, componentName(r.component()), innerX + 22, ry + 5, rightX - (innerX + 22), TEXT);
            }
        }
        g.disableScissor();
    ...
    private void renderBuildArea(GuiGraphicsExtractor g, int mouseX, int mouseY, SpellCombo combo) {
        ...
        for (SlotBox b : boxes) {
            if (b.kind() == SlotKind.EFFECT || b.kind() == SlotKind.ADD_EFFECT) {
                ...
                // horizontal stub from the effect slot
    ...
    private void renderComponentTooltip(GuiGraphicsExtractor g, Object comp, double mouseX, double mouseY) {
    ...
    private boolean acceptsDrag(SlotBox b, Object drag) {
        ...
        if (drag instanceof SpellModifier) return b.kind() == SlotKind.MODIFIER || b.kind() == SlotKind.ADD_MODIFIER;
        return false;
    }
