...
public class SpellVisualScreen extends Screen {
    ...
    protected void init() {
        ...
        // The sliders stay live in both colour modes: touching one is an unambiguous request for a
        // custom colour, so it switches the mode instead of doing nothing. A disabled slider only
        // reads as "this screen is broken" to a player who has already grabbed the handle.
        addRenderableWidget(new RgbSlider(left, top, fieldW, "R", r, v -> r = v, this::adoptCustomColor));
        addRenderableWidget(new RgbSlider(left, top + 22, fieldW, "G", g, v -> g = v, this::adoptCustomColor));
        addRenderableWidget(new RgbSlider(left, top + 44, fieldW, "B", b, v -> b = v, this::adoptCustomColor));

        colorModeButton = addRenderableWidget(Button.builder(defaultToggleLabel(), btn -> {
    ...
    /** A slider was touched: leave "from effect" mode and keep the toggle's label honest. */
    private void adoptCustomColor() {
        if (!useDefault) return;
        useDefault = false;
        if (colorModeButton != null) colorModeButton.setMessage(defaultToggleLabel());
    }
    ...
    public void render(GuiGraphics gfx, int mouseX, int mouseY, float partialTick) {
        ...
        gfx.renderOutline(swX, swY, swSize, swSize, PANEL_BORDER);

        super.render(gfx, mouseX, mouseY, partialTick);
    }

    /** Lightweight 2D silhouette of the chosen shape, tinted by the chosen color. */
    private void drawShapePreview(GuiGraphics gfx, int x, int y, int size, int color) {
    ...
    private static class RgbSlider extends AbstractSliderButton {
        ...
        RgbSlider(int x, int y, int w, String channel, int initial,
                  java.util.function.IntConsumer setter, Runnable onTouched) {
        ...
        protected void updateMessage() {
            // No explicit colour: AbstractSliderButton signals active/inactive purely through the
            // text colour it passes to renderScrollingString, and a hardcoded style overrides it.
        ...
        public void onClick(double mouseX, double mouseY) {
            // applyValue only fires when the value actually moves; a click that lands on the handle
            // where it already sits is still a request for a custom colour.
