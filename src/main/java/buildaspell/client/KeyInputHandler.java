...
public class KeyInputHandler {
    ...
    public static void handleKeyInput() {
        ...
        if (ModKeyBinds.CAST_SPELL.consumeClick()) {
            PacketDistributor.sendToServer(new CastSpellPacket());
        }
        ...
        if (ModKeyBinds.OPEN_SPELL_BUILDER.consumeClick()) {
            ...
            mc.setScreen(new SpellBuilderScreen(spellData, spellSlots));
        }

        // Open ability ring
        if (ModKeyBinds.OPEN_ABILITY_RING.consumeClick()) {
            mc.setScreen(new AbilityRingScreen());
        }

        // Import spell
        if (ModKeyBinds.IMPORT_SPELL.consumeClick()) {
            mc.setScreen(new SpellImportScreen(null, code -> {
                ...
                if (spell != null) {
                    ...
                    mc.setScreen(new SpellBuilderScreen(data, slots));
                }
            }));
        }
        ...
        if (ModKeyBinds.NEXT_SPELL_SLOT.consumeClick()) {
            ...
            PacketDistributor.sendToServer(new SetActiveSlotPacket(next));
        }
        if (ModKeyBinds.PREV_SPELL_SLOT.consumeClick()) {
            ...
            PacketDistributor.sendToServer(new SetActiveSlotPacket(prev));
        }
    }

    private static void handleSlotKey(net.minecraft.client.KeyMapping key, int slotIndex) {
        if (key.consumeClick()) {
            PacketDistributor.sendToServer(new SetActiveSlotPacket(slotIndex));
        }
    }
}
