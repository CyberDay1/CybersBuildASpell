...
public class RuneMarkerRenderer extends EntityRenderer<RuneMarkerEntity, RuneMarkerRenderState> {
    ...
    public void submit(RuneMarkerRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        final int finalRingR = ringR;
        final int finalRingG = ringG;
        final int finalRingB = ringB;
        collector.submitCustomGeometry(poseStack, RENDER_TYPE, (pose, consumer) -> {
            renderRing(consumer, pose, outerRadius, 0.05f, finalRingR, finalRingG, finalRingB, finalAlpha);
        });
        ...
        if (charge > 0.1f) {
            ...
            final int innerG = trap ? 40 : 220;
            final int innerB = trap ? 40 : 255;
            collector.submitCustomGeometry(poseStack, RENDER_TYPE, (pose, consumer) -> {
                renderFilledCircle(consumer, pose, finalInnerRadius, innerR, innerG, innerB, innerAlpha);
            });
        }
