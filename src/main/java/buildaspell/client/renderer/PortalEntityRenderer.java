...
public class PortalEntityRenderer extends EntityRenderer<PortalEntity> {
    ...
    private static final ResourceLocation TEXTURE_PORTAL =
    ...
    private static final RenderType RENDER_TYPE_PORTAL =
            RenderType.entityTranslucentEmissive(TEXTURE_PORTAL);
    ...
    public void render(PortalEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        Vec3 axisW  = entity.getAxisW();
        Vec3 axisH  = entity.getAxisH();
        Vec3 normal = entity.getNormal();

        // Arcane teal when dialed, pink when undialed
        int r = isDialed ? 60  : 255;
        int g = isDialed ? 210 : 80;
        int b = isDialed ? 255 : 180;

        VertexConsumer consumer = buffer.getBuffer(RENDER_TYPE_PORTAL);
        ...
        renderPortalLayer(consumer, poseStack.last(), axisW, axisH, normal,
