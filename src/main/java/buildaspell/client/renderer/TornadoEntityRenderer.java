...
public class TornadoEntityRenderer extends EntityRenderer<TornadoEntity, TornadoRenderState> {
    ...
    public void extractRenderState(TornadoEntity entity, TornadoRenderState state, float partialTick) {
        ...
        // Drive the funnel animation off the client-advancing render age, not the frozen custom
        // lifetime counter (getTickCount stays 0 client-side, which made a large tornado twitch
        // instead of spin). getRenderAge() exposes the inherited vanilla Entity.tickCount.
     ...
     * On 26.1 the culling box is supplied by the renderer (it moved off Entity), so it lives here.
     */
    @Override
