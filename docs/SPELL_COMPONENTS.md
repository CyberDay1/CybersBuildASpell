...
| `tracking` | 35.0 | Fires a homing projectile from the player's eye position (1.5 speed, no gravity). It takes hold of one living target and keeps it until that target dies, is struck, or leaves `homingRange`, then looks for another; it turns toward the target at `homingStrength` while holding the speed it was fired at, rather than accelerating |
...
|---|---|---|---|
| `damage` | 5.0 | Deals magic damage to entities in range | `spellPower × 0.20` |
...
| `light` | 12.0 | Fills a sphere around the spell's origin with invisible light sources that expire on their own | Power level increases brightness (12-15). Lights clear themselves after 60 seconds by default (`durationTicks`); the timer is stored with the chunk, so it survives a reload or a restart. Break also clears them |
...
| `break` | 40.0 | Breaks blocks within range and drops items | Power level increases max hardness breakable (base 3.0 + power × 2.0), Gentleness applies silk touch, Fortunate Son applies fortune. Also clears light blocks outright, which the hardness limit would otherwise skip: this is the only way to remove the permanent lights left by v1.0.0. With Fill it follows the mass instead of clearing a sphere: it walks from block to touching block, so it eats the vein or wall it struck and stops at the open air |
...
| `summon` | 50.0 | Summons tamed wolves at origin | Summons 1 + power level wolves (max 5), arranged in circle pattern. Like every summoned creature they are conjured out of mana, so they drop no items and no experience when they die |
| `create_water` | 20.0 | Places water through the spell's range | Fills a sphere of the spell's range, replacing air and any replaceable block. With Fill it pours instead: it settles to the floor and rises a layer at a time, stopping where it comes up to the surface — a hole fills to ground level and a room fills to its ceiling, as far as one cast's reach and block budget carry it |
| `evaporate_water` | 25.0 | Removes water source blocks within range | Removes every water source in a sphere of the spell's range. With Fill it follows the water instead: it drains the body of water it was cast into and stops at the shore |
...
| `conjure` | 25.0 | Creates blocks at spell origin | Builds with the caster's off-hand block if it is in `#buildaspell:conjurable`, otherwise places stone; the held block is never consumed. Fills air and any replaceable block (light, grass, snow, water), with or without Fill. Wall and Floor reshape it into a vertical wall or a horizontal platform; Fill makes it pour, settling to the floor and rising a layer at a time until it comes up to the surface — a hole fills to ground level and a room fills to its ceiling, as far as one cast's reach and block budget carry it |
...
| `fortunate_son` | 25.0 | Yes | Adds fortune level to Break effect (ore drops), Reap effect (crop drops), and looting level to spell damage (mob drops). Each stack provides +1 Fortune/Looting level, and the mana cost is what limits how many you can afford: every repeat costs 1.5x the one before it (`shared.repeatCostGrowth`), so a fourth stack costs 3.4x a single one and a seventh costs 11.4x. `maxLevel` (25, server-configurable) is a backstop rather than the balance lever — against the Mana Pool attribute's own ceiling of 10000 a caster runs out of mana at around 13 stacks, or 14 holding a Runic wand. The level is applied as the real enchantment — a Fortune tool for blocks and crops, a Looting level lent to the killer for the duration of the death loot roll — so it only ever multiplies what a loot table declares as fortune- or looting-affected. Guaranteed drops (a boss's trophy, a mob's own equipment) are untouched, and a player killed by the spell drops their own inventory, which is left alone |
| `double` | +80% of the spell | Yes | Increases projectile count by 1 per stack for Cast/Tracking delivery methods (projectiles spread horizontally). Charges no flat price: each stack multiplies the whole spell's mana cost by `totalCostMultiplier` (1.8, server-configurable), because a second projectile doubles everything the spell does. Stacks compound, so two are 1.8 × 1.8 |
| `echo` | +80% of the spell | Yes | Re-casts the spell after a short delay (10 ticks per echo); each echo strikes at reduced power (×0.8 per echo). Works with every delivery method. Priced like `double`: no flat cost, each stack multiplies the whole spell's mana cost by `totalCostMultiplier` (1.8, server-configurable) and stacks compound |
...
| `fill` | 40.0 | No | Fills the space the spell lands in, and stops where that space stops. The spell walks from block to touching block rather than measuring distance, so it cannot reach through a wall into a room it never touched. Conjure and Create Water pour: they settle to the floor and rise a layer at a time, so a hole fills to ground level and a sealed room fills to its ceiling — cast somewhere genuinely open there is nothing holding the fill, and nothing is placed. Break and Evaporate Water follow the mass instead: Break clears the run of blocks it hit and stops at the open air, Evaporate drains the body of water it was cast into and stops at its shore. The radius (30 by default: `fillRadiusBase` 25 + `fillRadiusPerFill` 5) bounds how far the search may travel in every direction — out, up and down from where the spell landed — rather than carving it to a shape. Running out of it stops that layer spreading rather than ending the cast, and only a layer that ran out of it is asked what is overhead, so an ordinary room is held by its walls whatever state its roof is in. `fillBudget` (4096) caps how many blocks one cast places, so a hall far wider than the radius fills as much of itself as a single cast can carry. Both are per effect and server-configurable. Overrides Increased Area; Wall and Floor still take priority over Fill |
...
| `sunder` | 25.0 | Yes | Raises the Damage effect's damage against an armored target, in proportion to how much armor it wears: a target in full diamond takes the whole increase, a half-armored one takes half of it, and an unarmored one takes none. Each stack raises it further, up to `maxMultiplier` (3.0 by default), which is what keeps several copies on one effect from running away. Because it multiplies the spell's own damage rather than adding a figure of its own, it scales with Spell Power. `bonusFractionPerLevel` (0.35) and `fullArmorValue` (20.0) tune it. Only affects the Damage effect |
...
- **Projectile Behavior**: Tracking projectiles lock onto the nearest living entity within 16 blocks (`homingRange`) and keep that target until it dies, is struck, or leaves range, turning toward it each tick by `homingStrength` (0.3, a fraction of the heading) while holding the speed they were fired at; Cast projectiles fly straight
...
- **Damage Effect**: `spellPower × 0.20` damage per hit. Repeating Damage in one spell sums into a
  single hit rather than landing one hit per copy, so armor, Protection and Resistance apply to the
  total.
