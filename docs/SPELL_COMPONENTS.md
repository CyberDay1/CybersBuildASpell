...
| `create_water` | 20.0 | Places water through the spell's range | Fills a sphere of the spell's range, replacing air and any replaceable block. With Fill it pours instead: it settles to the floor and rises a layer at a time, stopping at the height where the space would spill out |
...
| `conjure` | 25.0 | Creates blocks at spell origin | Copies target block if allowed in config, otherwise places stone. Fills air and any replaceable block (light, grass, snow, water), with or without Fill. Wall and Floor reshape it into a vertical wall or a horizontal platform; Fill makes it pour, settling to the floor and rising a layer at a time until the space would spill out |
...
| `fortunate_son` | 25.0 | Yes | Adds fortune level to Break effect (ore drops), Reap effect (crop drops), and looting level to spell damage (mob drops). Each stack provides +1 Fortune/Looting level, up to a cap of 3 (`maxLevel`, server-configurable). Only creature loot is affected: a player killed by the spell drops their own inventory, which is left untouched. Bonus drops never exceed an item's own stack limit |
