...
| `break` | 40.0 | Breaks blocks within range and drops items | Power level increases max hardness breakable (base 3.0 + power × 2.0), Gentleness applies silk touch, Fortunate Son applies fortune. Also clears light blocks outright, which the hardness limit would otherwise skip: this is the only way to remove the permanent lights left by v1.0.0 |
...
| `conjure` | 25.0 | Creates blocks at spell origin | Copies target block if allowed in config, otherwise places stone. Fills air and any replaceable block (light, grass, snow, water), with or without Fill. Works with wall/floor/fill modifiers to control shape |
...
| `fill` | 40.0 | No | Reshapes the spell's area into a solid sphere around the spell's origin: radius 30 by default (`fillRadiusBase` 25 + `fillRadiusPerFill` 5, set per effect and server-configurable). **Ignores line of sight**: the sphere is measured by distance alone, so it passes straight through walls and floors into sealed rooms, caves and anything else behind solid blocks. Overrides Increased Area, which does not widen the Fill radius. Shapes Conjure (fills air and replaceable blocks), Break (clears the volume), Create Water and Evaporate Water |
